...
public class SetNewPassword extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		result.addProperty(Constants.result, "Reset code, username or password is incorrect");
		...
		try {
			...
			if (Validator.hasValidResetCredentials(loginCredentials)) {
				...
				reseting:{
					...
					if (reset == null){
						result.addProperty(Constants.result, "No reset request found for user " + loginCredentials.getUsername() + ".");
						break reseting;
					}
					
					if (! reset.getCode().equals( loginCredentials.getCode() ) ){
						result.addProperty(Constants.result, "Submitted reset code does not match registerd code. Please check spelling.");
						break reseting;
					}
					...
					if ( then.before( reset.getCreated() ) ){
						result.addProperty(Constants.result, "Reset has expired (valid for " + validMinutes + " minutes. Try again.");
						break reseting;
					}
					
					if (! Users.clearReset(loginCredentials)){
						result.addProperty(Constants.result, "Something went wrong removing the reset. Try again");
						break reseting;
					}
					...
					if (! Users.setPassword(loginCredentials)) {
						result.addProperty(Constants.result, "Something went wrong setting the new password. Try again");
						break reseting;						
					}
